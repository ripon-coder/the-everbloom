                ...
                <div class="flex items-center gap-2.5 text-gray-300">
                    @if($site_setting && !empty($site_setting->getSocialLinks()))
                        @foreach($site_setting->getSocialLinks() as $key => $social)
                            <a href="{{ $social['url'] }}" target="_blank" rel="noopener noreferrer" class="hover:text-white transition-colors flex items-center justify-center" title="{{ $social['name'] }}" aria-label="{{ $social['name'] }}">
                                {!! str_replace('class="w-4 h-4"', 'class="w-3.5 h-3.5"', $social['svg']) !!}
                            </a>
                        @endforeach
                    @endif
                </div>
