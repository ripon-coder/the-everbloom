<div x-data="{
    ...
    <a href="{{ route('shop') }}" class="flex flex-col items-center gap-1 {{ request()->routeIs('shop') ? 'text-primary' : 'text-slate-700' }} hover:text-primary transition-colors">
        ...
        <span class="text-[10px] font-semibold uppercase tracking-wide">Shop</span>
    </a>

    <button @click="if (!window.location.pathname.includes('/checkout')) { $store.cartDrawer ? $store.cartDrawer.open() : window.dispatchEvent(new CustomEvent('open-cart-drawer')) }" class="flex flex-col items-center gap-1 {{ request()->routeIs('cart') ? 'text-primary' : 'text-slate-700' }} hover:text-primary transition-colors relative">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <span class="text-[10px] font-semibold uppercase tracking-wide">Cart</span>
