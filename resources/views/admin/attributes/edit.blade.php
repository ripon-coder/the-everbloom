                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <nav class="flex items-center space-x-2 text-xs text-gray-500 mb-1.5">
                            ...
                            <span class="text-gray-700 font-semibold truncate max-w-[200px]">{{ $attribute->name }}</span>
                            <span class="text-gray-300">/</span>
                            <span class="text-gray-500">Edit</span>
                        </nav>
                        <div class="flex flex-wrap items-center gap-3">
                            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Attribute</h1>
            ...
            <form action="{{ route('admin.attributes.update', $attribute->id) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 pt-4 border-t border-gray-200">
                        ...
                        <div>
                            ...
                            <label class="p-3 border border-gray-200 bg-gray-50/50 flex items-start space-x-3 cursor-pointer">
                                <input type="checkbox" name="is_image" id="is_image" value="1"
                                       {{ old('is_image', $attribute->is_image) ? 'checked' : '' }}
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                      ...
                                      class="w-full px-3 py-2 text-xs border @error('description') border-rose-500 @else border-gray-300 @enderror bg-white focus:ring-1 focus:ring-blue-600 focus:border-blue-600">{{ old('description', $attribute->description ?? '') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-[11px] text-rose-600 font-medium">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="px-5 sm:px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                    <div class="text-gray-500 space-x-3">
                        <span>Created: {{ $attribute->created_at ? $attribute->created_at->format('M d, Y · H:i') : 'N/A' }}</span>
                        <span>&bull;</span>
                        <span>Updated: {{ $attribute->updated_at ? $attribute->updated_at->format('M d, Y · H:i') : 'N/A' }}</span>
                    </div>
