...
@section('content')
    <div class="p-4 dark:bg-gray-900">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Flash Sale</h1>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="slug" name="slug" value="{{ old('slug', $flashSale->slug) }}" 
                               placeholder="Enter flash sale slug"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <div>
                        ...
                        <input type="datetime-local" id="start_date" name="start_date" 
                               value="{{ old('start_date', $flashSale->start_date->format('Y-m-d\TH:i')) }}" required
                    ...
                    <div>
                        ...
                        <input type="datetime-local" id="end_date" name="end_date" 
                               value="{{ old('end_date', $flashSale->end_date->format('Y-m-d\TH:i')) }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <div>
                        ...
                        <select id="status" name="status" required
                            ...
                            @foreach ($status_options as $value => $label)
                                <option value="{{ $value }}" {{ old('status', $flashSale->status) == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" id="banner_image" name="banner_image" 
                               value="{{ old('banner_image', $flashSale->banner_image) }}"
                ...
                <div class="mt-6">
                    ...
                    <textarea id="description" name="description" rows="3"
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200 bg-white dark:bg-gray-700 text-gray-900 dark:text-white @error('description') border-red-500 @enderror">{{ old('description', $flashSale->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <div>
                        ...
                        <p class="text-base text-gray-900 dark:text-white">{{ $flashSale->created_at->format('M d, Y H:i') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Updated At</label>
                        <p class="text-base text-gray-900 dark:text-white">{{ $flashSale->updated_at->format('M d, Y H:i') }}</p>
                    </div>
                </div>
            </div>
