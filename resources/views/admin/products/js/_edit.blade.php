...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.addEventListener('click', function(e) {
            ...
            // Delete variant image button
            if (e.target.closest('.delete-variant-image')) {
                const button = e.target.closest('.delete-variant-image');
                const imageId = button.dataset.imageId;
                const variantIndex = button.dataset.variantIndex;
                
                if (confirm('Are you sure you want to delete this image?')) {
                    // Find the image container and remove it
                    const imageContainer = document.getElementById(`variant-image-container-${imageId}`);
                    if (imageContainer) {
                        imageContainer.remove();
                    }
                    
                    // Set the hidden input to mark image for deletion
            ...
            // Remove variant button (for existing variants)
            if (e.target.closest('.remove-variant')) {
                const button = e.target.closest('.remove-variant');
                const variant = button.closest('[data-variant]');
                
                if (confirm('Are you sure you want to remove this variant?\nNote: Before updating, you can undo by refreshing the page.')) {
                    // Check if this is an existing variant (has ID)
                    const variantIdInput = variant.querySelector('input[name$="[id]"]');
                    if (variantIdInput && variantIdInput.value) {
                        // Add to hidden delete list
                        ...
                        deleteInput.value += (deleteInput.value ? ',' : '') + variantIdInput.value;
                    }
        ...
        <div class="p-6">
            ...
            <div>
                <div class="flex items-center mb-4">
                    ...
                    <h4 class="text-base font-semibold text-gray-900 dark:text-white">Variant Image</h4>
                </div>
