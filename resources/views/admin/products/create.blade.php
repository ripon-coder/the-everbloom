...
<div class="bg-white dark:bg-gray-800 shadow-md rounded-lg max-w-4xl mx-auto">
    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
        <h2 class="text-xl font-semibold text-gray-800 dark:text-white">Add New Product</h2>
        ...
        <div class="grid gap-6 mb-6 md:grid-cols-2">
            ...
            <div>
                ...
                <select name="status" id="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 @error('status') border-red-500 @enderror" required>
                    <option value="{{ \App\Constants\ProductStatus::ACTIVE }}" {{ old('status') == \App\Constants\ProductStatus::ACTIVE ? 'selected' : '' }}>Active</option>
                    <option value="{{ \App\Constants\ProductStatus::INACTIVE }}" {{ old('status') == \App\Constants\ProductStatus::INACTIVE ? 'selected' : '' }}>Inactive</option>
                </select>
                @error('status')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops!</span> {{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                @error('image_path')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops!</span> {{ $message }}</p>
                @enderror
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400" id="file_input_help">Allowed types: jpg, jpeg, png, gif. Max size: 2MB.</p>
            </div>
        </div>
        <div class="mb-6">
