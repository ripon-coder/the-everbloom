...
<div class="p-4">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex space-x-3">
            <a href="{{ route('admin.products.edit', $product) }}" class="bg-yellow-600 hover:bg-yellow-700 text-white font-medium py-2 px-4 rounded-lg transition duration-200 flex items-center">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p class="text-base text-gray-900 font-semibold">${{ number_format($product->price, 2) }}</p>
                    </div>
                    <div>
                                        ...
                                        <div class="flex flex-wrap gap-2">
                                            @foreach($variant->variantAttributes as $variantAttribute)
                                                <span class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-gray-100 text-gray-800">
                                                    {{ $variantAttribute->attribute?->name }}: {{ $variantAttribute->attributeValue?->value }}
                                                </span>
                                            @endforeach
                                        </div>
                                    </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-lg font-semibold text-green-600">{{ $product->variants->where('status', 'active')->count() }}</span>
                    </div>
                    
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-gray-600">Total Stock</span>
                        <span class="text-lg font-semibold text-gray-900">{{ $product->variants->sum('stock') }}</span>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="space-y-3">
                    <a href="{{ route('admin.products.edit', $product) }}" class="w-full bg-yellow-600 hover:bg-yellow-700 text-white font-medium py-2 px-4 rounded-lg transition duration-200 flex items-center justify-center">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                        </svg>
                        Edit Product
                    ...
                    @if($product->trashed())
                        <form action="{{ route('admin.products.restore', $product->id) }}" method="POST" class="inline w-full">
                            @csrf
                            <button type="submit" 
                        ...
                        <form action="{{ route('admin.products.force-delete', $product->id) }}" method="POST" class="inline w-full">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
                    ...
                    @else
                        <form action="{{ route('admin.products.destroy', $product) }}" method="POST" class="inline w-full">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
                                    class="w-full bg-red-600 hover:bg-red-700 text-white font-medium py-2 px-4 rounded-lg transition duration-200 flex items-center justify-center"
