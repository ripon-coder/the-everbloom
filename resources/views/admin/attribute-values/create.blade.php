...
<div class="bg-white dark:bg-gray-800 shadow-md rounded-lg max-w-4xl mx-auto">
    ...
    <form action="{{ route('admin.attribute-values.store') }}" method="POST" class="p-6">
        ...
        <div class="mb-6">
            ...
            <input type="text" name="value" id="value" value="{{ old('value') }}" placeholder="Enter value (e.g., Red, Blue, S, M)" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 @error('value') border-red-500 @enderror">
            @error('value')
                <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops!</span> {{ $message }}</p>
            @enderror
        </div>
