    ...
    <form action="{{ route('admin.attribute-values.update', $attributeValue->id) }}" method="POST" class="p-6">
            ...
            <div>
                ...
                <select name="product_id" id="product_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 @error('product_id') border-red-500 @enderror" required>
                    ...
                    @foreach ($products as $product)
                        <option value="{{ $product->id }}" {{ old('product_id', $attributeValue->product_id) == $product->id ? 'selected' : '' }}>
                            {{ $product->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="mb-6">
            ...
            <div id="single-value-container" class="mb-4">
                <input type="text" name="value" id="value" value="{{ old('value', $attributeValue->value) }}" placeholder="Enter attribute value" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 @error('value') border-red-500 @enderror">
                @error('value')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops!</span> {{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 mb-6">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">Attribute Value Information</h3>
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                    <span class="font-medium text-gray-700 dark:text-gray-300">ID:</span>
                    <span class="text-gray-600 dark:text-gray-400">{{ $attributeValue->id }}</span>
                </div>
                <div>
                    <span class="font-medium text-gray-700 dark:text-gray-300">Created:</span>
                    <span class="text-gray-600 dark:text-gray-400">{{ $attributeValue->created_at->format('Y-m-d H:i:s') }}</span>
                </div>
                <div>
                    <span class="font-medium text-gray-700 dark:text-gray-300">Last Updated:</span>
                    <span class="text-gray-600 dark:text-gray-400">{{ $attributeValue->updated_at->format('Y-m-d H:i:s') }}</span>
                </div>
                <div>
                    ...
                    <span class="text-gray-600 dark:text-gray-400">
                        @if($attributeValue->values && is_array($attributeValue->values))
                            {{ implode(', ', $attributeValue->values) }}
                        @else
                            {{ $attributeValue->value ?: 'Empty' }}
                        @endif
                    </span>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Current attribute value data
    const currentValues = {{ $attributeValue->values ? json_encode($attributeValue->values) : '[]' }};
    ...
    function updateValueInterface(attributeId) {
        ...
        if (attributeType === 'checkbox' || attributeType === 'select-multiple') {
            ...
            options.forEach(option => {
                ...
                div.innerHTML = `
                    <input type="checkbox" id="option_${option.value}" name="option_values[]" value="${option.value}" ${isChecked ? 'checked' : ''} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
