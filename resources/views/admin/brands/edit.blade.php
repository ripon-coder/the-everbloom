...
@section('content')
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg max-w-4xl mx-auto">
        ...
        <form action="{{ route('admin.brands.update', $brand->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug"
                        ...
                        value="{{ old('slug', $brand->slug) }}" placeholder="brand-name" required>
                ...
                <div>
                    ...
                    <select name="status" id="status"
                        ...
                        required>
                        <option value="{{ \App\Constants\BrandStatus::ACTIVE }}"
                            {{ old('status', $brand->status) == \App\Constants\BrandStatus::ACTIVE ? 'selected' : '' }}>
                            Active</option>
                        <option value="{{ \App\Constants\BrandStatus::INACTIVE }}"
                            {{ old('status', $brand->status) == \App\Constants\BrandStatus::INACTIVE ? 'selected' : '' }}>
                            Inactive</option>
                    </select>
                    @error('status')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops!</span>
            ...
            <div>
                <label for="logo" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Brand
                    Logo</label>
                <input type="file" name="logo" id="logo"
                    class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 @error('logo') border-red-500 @enderror"
                    accept="image/*" onchange="previewLogo(this)">
                <div class="mt-2 flex items-center space-x-3">
                    <img id="logo-preview" 
                        src="{{$brand->getImageUrl('brand_logo')}}"
                    ...
                    <span class="text-sm text-gray-600 dark:text-gray-400">
                        @if ($brand->getImageUrl('brand_logo'))
                            Current logo (upload new image to replace)
                        @else
                            Default logo (will be replaced when you upload an image)
                        @endif
                    </span>
                </div>
                @error('logo')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops!</span>
                        {{ $message }}</p>
                @enderror
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Leave empty to keep current logo. Allowed
            ...
            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 mb-6">
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <span
                            class="text-gray-900 dark:text-white ml-2">{{ $brand->created_at->format('Y-m-d H:i') }}</span>
                    </div>
                    <div>
                        <span class="text-gray-600 dark:text-gray-400">Last Updated:</span>
                        <span
                            class="text-gray-900 dark:text-white ml-2">{{ $brand->updated_at->format('Y-m-d H:i') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-between items-center">
                ...
                <div class="flex space-x-3">
                    <button type="button"
                        onclick="event.preventDefault(); showDeleteModal('brand', '{{ route('admin.brands.destroy', $brand->id) }}', '{{ $brand->name }}')"
