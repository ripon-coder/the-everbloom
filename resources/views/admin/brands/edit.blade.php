                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <nav class="flex items-center space-x-2 text-xs text-gray-500 mb-1.5">
                            ...
                            <span class="text-gray-700 font-semibold truncate max-w-[200px]">{{ $brand->name }}</span>
                            <span class="text-gray-300">/</span>
            ...
            <form action="{{ route('admin.brands.update', $brand->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="p-5 sm:p-6 space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <select name="status" id="status" required
                                    ...
                                    <option value="{{ \App\Constants\BrandStatus::ACTIVE }}" {{ old('status', $brand->status) == \App\Constants\BrandStatus::ACTIVE ? 'selected' : '' }}>
                                        Active
                                    </option>
                                    <option value="{{ \App\Constants\BrandStatus::INACTIVE }}" {{ old('status', $brand->status) == \App\Constants\BrandStatus::INACTIVE ? 'selected' : '' }}>
                                        Inactive
                                    </option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="p-3 border border-dashed border-gray-300 bg-gray-50/50 flex items-center gap-3">
                                    <img id="logo-preview" src="{{ $brand->getImageUrl('brand_logo') }}" alt="{{ $brand->name }} Logo Preview"
                                    ...
                                    <div class="space-y-1 text-xs">
                                        ...
                                        <p class="text-[10px] text-gray-500">Leave empty to keep existing logo. Formats: JPG, PNG, SVG, WEBP. Max: 2MB.</p>
                                    </div>
                                </div>
                                @error('logo')
                                    <p class="mt-1 text-[11px] text-rose-600 font-medium">{{ $message }}</p>
                    ...
                    <div class="pt-4 border-t border-gray-200">
                        ...
                        <textarea name="description" id="description" rows="3"
                                  ...
                                  class="w-full px-3 py-2 text-xs border @error('description') border-rose-500 @else border-gray-300 @enderror bg-white focus:ring-1 focus:ring-blue-600 focus:border-blue-600">{{ old('description', $brand->description ?? '') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-[11px] text-rose-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="px-5 sm:px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                    <div class="text-gray-500 space-x-3">
                        <span>Created: {{ $brand->created_at ? $brand->created_at->format('M d, Y · H:i') : 'N/A' }}</span>
                        <span>&bull;</span>
                        <span>Updated: {{ $brand->updated_at ? $brand->updated_at->format('M d, Y · H:i') : 'N/A' }}</span>
                    </div>
