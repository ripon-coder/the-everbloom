<div class="bg-white border border-gray-200 rounded-md overflow-hidden" x-data="{ 
    ...
    <div class="p-4 md:p-6 border-b border-gray-100 flex items-center justify-between">
        ...
        <span class="bg-red-50 text-red-600 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider">
    ...
    <div class="p-4 md:p-6">
        @if($wishlist->isEmpty())
            <div class="py-12 flex flex-col items-center justify-center text-center">
                ...
                <a href="{{ route('shop') }}" class="bg-slate-900 text-white px-6 py-2.5 rounded-md font-bold text-xs uppercase tracking-widest hover:bg-red-600 transition-colors">Start Shopping</a>
            </div>
        @else
            <div class="space-y-4">
                    ...
                    <div class="wishlist-item flex flex-col sm:flex-row items-center gap-4 p-4 border border-gray-100 rounded-md hover:border-gray-200 transition-all duration-300">
                        ...
                        <div class="flex-1 min-w-0 text-center sm:text-left">
                            <h4 class="text-sm md:text-base font-bold text-gray-900 truncate mb-1">
                                <a href="{{ route('product.show', $item->product->slug) }}" class="hover:text-red-600 transition-colors">
                                    {{ $item->product->name }}
                                </a>
                            </h4>
                        ...
                        <div class="flex items-center gap-3">
                            <a href="{{ route('product.show', $item->product->slug) }}" 
                               class="bg-slate-900 text-white px-5 py-2.5 rounded-md text-[10px] font-black uppercase tracking-widest hover:bg-red-600 transition-colors whitespace-nowrap">
                                View Product
                            </a>
                            <button @click="removeItem({{ $item->product_id }}, $el)" 
                                    class="p-2.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-full transition-all duration-200"
