<div class="bg-white border border-gray-200 rounded-none overflow-hidden" x-data="{ 
    ...
    <div class="p-4 md:p-6 border-b border-gray-100 flex items-center justify-between">
        <h2 class="text-base md:text-lg font-bold text-slate-900 uppercase tracking-tight">My Wishlist</h2>
        <span class="bg-primary-50 text-primary text-[10px] font-bold px-2.5 py-1 rounded-none uppercase tracking-wider">
    ...
    <div class="p-4 md:p-6">
        @if($wishlist->isEmpty())
            <div class="py-12 flex flex-col items-center justify-center text-center">
                ...
                <a href="{{ route('shop') }}" class="bg-slate-900 text-white px-6 py-2.5 rounded-none font-bold text-xs uppercase tracking-widest hover:bg-primary transition-colors">Start Shopping</a>
            </div>
        @else
            <div class="space-y-4">
                @foreach($wishlist as $item)
                    <div class="wishlist-item flex flex-col sm:flex-row items-center gap-4 p-4 border border-gray-200 rounded-none hover:border-gray-300 transition-all duration-300 bg-white">
                        ...
                        <div class="flex-1 min-w-0 text-center sm:text-left">
                            <h4 class="text-sm md:text-base font-bold text-slate-900 truncate mb-1">
                                <a href="{{ route('product.show', $item->product->slug) }}" class="text-slate-900 hover:text-primary transition-colors block font-bold">
                                    {{ $item->product->name }}
                                </a>
                            </h4>
                            @php
                            ...
                            <div class="flex items-center justify-center sm:justify-start gap-2">
                                <span class="text-sm md:text-base font-black text-emerald-600" style="color: #059669 !important;">Tk. {{ number_format((float)$price, 2) }}</span>
                        ...
                        <div class="flex items-center gap-3">
                            <a href="{{ route('product.show', $item->product->slug) }}" 
                               class="bg-slate-900 text-white px-5 py-2.5 rounded-none text-[10px] font-black uppercase tracking-widest hover:bg-primary transition-colors whitespace-nowrap">
                                View Product
                            </a>
                            <button @click="removeItem({{ $item->product_id }}, $el)" 
                                    class="p-2.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-none transition-all duration-200"
