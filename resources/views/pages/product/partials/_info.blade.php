<div class="flex flex-col">
    <div class="mb-2 md:mb-4">
        ...
        <div class="flex items-center gap-3 text-xs md:text-sm">
            <div class="flex text-amber-400">
                @for($i=0; $i<5; $i++)
                    <svg class="w-3.5 h-3.5 md:w-4 md:h-4 fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                @endfor
            </div>
        ...
        <div class="mt-2 flex flex-wrap gap-3 text-[10px] md:text-xs">
            <div class="flex items-center gap-1.5">
                ...
                <span class="bg-gray-100 text-gray-600 px-2 py-0.5 rounded-md font-medium">{{ $product->sku ?? 'N/A' }}</span>
            </div>
            <div class="flex items-center gap-1.5">
                <span class="font-bold text-gray-400 uppercase tracking-tighter">Category:</span>
                <span class="bg-gray-100 text-gray-600 px-2 py-0.5 rounded-md font-medium">{{ $product->category->name ?? 'Uncategorized' }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-4">
        ...
        <div class="hidden md:flex flex-col sm:flex-row gap-2 pt-2">
            <button @click="addToCart($event)" class="flex-1 bg-red-600 hover:bg-red-700 text-white font-bold py-3.5 px-4 rounded-md text-xs uppercase tracking-wide transition-colors flex items-center justify-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z"/></svg>
                Add to Cart
            </button>
            <button class="flex-1 bg-gray-900 hover:bg-black text-white font-bold py-3.5 px-4 rounded-md text-xs uppercase tracking-wide transition-colors">
                Buy It Now
            </button>
        </div>
