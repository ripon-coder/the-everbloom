            ...
            @foreach($relatedProducts->take(5) as $relProduct)
                <a href="{{ route('product.show', $relProduct->slug) }}" class="flex gap-4 group items-center">
                    ...
                    <div class="flex flex-col gap-1.5 flex-1 min-w-0">
                        <h4 class="text-sm sm:text-base font-bold text-gray-800 line-clamp-2 group-hover:text-primary transition-colors leading-snug">{{ $relProduct->name }}</h4>
                        <div class="flex flex-wrap items-center gap-2">
                            <span class="text-base font-bold text-primary">Tk.{{ number_format($relProduct->price) }}</span>
                            @if($relProduct->old_price)
                                <span class="text-xs sm:text-sm text-gray-400 line-through">Tk.{{ number_format($relProduct->old_price) }}</span>
                            @endif
                        </div>
                    </div>
                </a>
