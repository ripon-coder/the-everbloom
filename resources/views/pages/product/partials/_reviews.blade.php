    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-gray-50 border border-gray-100 p-5 md:p-6">
            ...
            @if($reviewCount > 0)
                <div class="flex items-start gap-5">
                    ...
                    <div class="text-center flex-shrink-0">
                        <div class="text-4xl font-bold text-gray-900 leading-none">{{ number_format($avgRating, 1) }}</div>
                        ...
                        <div class="text-xs text-gray-500 mt-1 font-medium">{{ $reviewCount }} {{ Str::plural('review', $reviewCount) }}</div>
                    </div>
                    ...
                    <div class="flex-1 space-y-1.5">
                        @for($r = 5; $r >= 1; $r--)
                            ...
                            <div class="flex items-center gap-2">
                                <span class="text-xs sm:text-sm font-bold text-gray-600 w-3 text-right">{{ $r }}</span>
                                <svg class="w-3.5 h-3.5 text-amber-400 fill-current flex-shrink-0" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                <div class="flex-1 h-2.5 bg-gray-200 overflow-hidden">
                                    <div class="h-full bg-amber-400 transition-all duration-500" style="width: {{ $pct }}%"></div>
                                </div>
                                <span class="text-xs text-gray-500 font-medium w-5 text-right">{{ $ratingCounts[$r] }}</span>
                            </div>
                        @endfor
                    </div>
                </div>
            @else
                <div class="flex flex-col items-center justify-center py-6 text-gray-400">
                    ...
                    <p class="text-xs sm:text-sm font-medium">No ratings yet</p>
                </div>
            @endif
        </div>
        ...
        <div class="bg-gray-50 border border-gray-100 p-5 md:p-6">
            ...
            <div x-show="localMessage" x-cloak
                 ...
                 class="mb-4 p-3 flex items-center gap-2 border text-xs font-bold uppercase tracking-wider"
                 :class="{
            ...
            @auth
                <form @submit.prevent="submitReview()" class="space-y-3">
                    <!-- Star Rating -->
                    <div>
                        <label class="text-xs font-bold text-gray-500 uppercase block mb-1">Your Rating <span class="text-danger">*</span></label>
                        <div class="flex gap-0.5 py-1">
                            ...
                            @endfor
                            <span class="ml-2 text-xs font-bold text-gray-500 self-center" x-show="rating > 0" x-text="['', 'Poor', 'Fair', 'Good', 'Very Good', 'Excellent'][rating]"></span>
                        </div>
                    </div>

                    <!-- Review Text -->
                    <div>
                        <label class="text-xs font-bold text-gray-500 uppercase block mb-1">Your Review <span class="text-danger">*</span></label>
                        <textarea x-model="reviewText" rows="3" class="w-full border border-gray-200 text-xs sm:text-sm focus:border-primary focus:ring-0 disabled:bg-gray-100 disabled:text-gray-400 resize-none p-3" placeholder="Share your experience with this product..." :disabled="isSubmitting" required></textarea>
                    </div>

                    <!-- Submit Button -->
                    <button type="submit" 
                        :disabled="rating === 0 || isSubmitting" 
                        class="w-full bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold py-3 text-xs sm:text-sm uppercase tracking-wide transition-colors flex items-center justify-center gap-2 shadow-xs">
            ...
            @else
                <div class="text-center py-6 bg-white border border-dashed border-gray-200">
                    ...
                    <p class="text-xs sm:text-sm text-gray-500 mb-3">Login to share your review</p>
                    <a href="{{ route('login') }}" class="inline-block bg-primary hover:bg-primary-dark text-white font-bold py-2.5 px-8 text-xs uppercase tracking-wide transition-colors">
                        Login to Review
                    </a>
                </div>
            @endauth
            ...
            @forelse($product->reviews as $review)
                <div class="py-5">
                    <div class="flex items-start gap-3">
                        <!-- Avatar -->
                        <div class="w-10 h-10 bg-primary/10 text-primary flex items-center justify-center text-xs sm:text-sm font-bold flex-shrink-0 uppercase">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center justify-between mb-1">
                                <div class="flex items-center gap-2">
                                    <span class="text-xs sm:text-sm font-bold text-gray-900">{{ $review->user->name ?? 'Customer' }}</span>
                                    @if($review->rating >= 4)
                                        <span class="bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-bold px-1.5 py-0.5 uppercase">Verified</span>
                                    @endif
                                </div>
                                <span class="text-xs text-gray-400 font-medium flex-shrink-0">{{ $review->created_at ? $review->created_at->format('M d, Y') : '' }}</span>
                            </div>
                            <!-- Stars -->
                            <div class="flex items-center gap-1 mb-2">
                                ...
                                <span class="text-xs font-bold text-gray-600">{{ ['', 'Poor', 'Fair', 'Good', 'Very Good', 'Excellent'][$review->rating] ?? '' }}</span>
                            </div>
                            <!-- Review Text -->
                            <p class="text-xs sm:text-sm text-gray-700 leading-relaxed">{{ $review->review }}</p>
                        </div>
                    </div>
                </div>
            @empty
                <div class="text-center py-12">
                    ...
                    <p class="text-xs sm:text-sm text-gray-500 font-medium">No reviews yet</p>
